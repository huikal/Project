package com.example.simplelogin.configuration;

import com.example.simplelogin.security.jwt.AuthEntryPointJwt;
import com.example.simplelogin.security.jwt.AuthTokenFilter;
...
//  스프링 시큐리티 룰을 무시하게 하는 Url 규칙(여기 등록하면 규칙 적용하지 않음)
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.cors()
        ...
        .authorizeRequests().antMatchers("/api/auth/**","/api/**").permitAll() // 이 url 은 모든 사용자 접근 허용
        .antMatchers("/api/test/**","/api/**").permitAll() // 이 url 은 모든 사용자 접근 허용
